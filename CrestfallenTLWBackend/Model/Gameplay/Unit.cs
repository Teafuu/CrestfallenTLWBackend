...
public class Unit : INavigator
{
    ...
    public string Move()
    {
        ...
        if(Vector2.Distance(Position, Waypoints[CurrentWayPointDestination].Position) <= 0.1) // waii 0.3?
        {
        ...
        return $"!{Key}|{Position.X.ToString("00.000", CultureInfo.InvariantCulture)}|{Position.Y.ToString("00.000", CultureInfo.InvariantCulture)}";
    } // CmdMoveUnits:!31|1.2|0.1!32|4.1|5.3
    ...
    private Tile[] Solve(Tile start)
    {
        ...
        var recordedResult = new Tile[Grid.TilesAsList.Count];
